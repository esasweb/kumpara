...
const String _kSyncDoneKey = 'onesignal_sync_done';
...
class MyApp extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return MaterialApp(
      ...
      theme: ThemeData(
        colorScheme: ColorScheme.fromSeed(seedColor: const Color(0xFF635BEA)),
        useMaterial3: true,
      ),
...
class _WebViewPageState extends State<WebViewPage> with WidgetsBindingObserver {
  ...
  String get appOpenAdUnitId {
    if (Platform.isAndroid) {
      return 'ca-app-pub-6275851890605245/4049081947';
    } else if (Platform.isIOS) {
      return 'ca-app-pub-6275851890605245/8084529874';
    }
    return '';
  }
  ...
  void _showAppOpenAdIfReady() {
    // 🔴 Giriş yapılmadıysa çıkış yap
    if (!_appOpenEnabled || !_isLoggedIn) return; 
    if (_appOpenAd == null) {
      _loadAppOpenAd();
      return;
    }
  ...
  void _maybeShowAd() async {
    ...
    // 🔴 user_id yoksa veya giriş kapalıysa kesinlikle gösterme
    if (userId == null || userId.isEmpty || !_isLoggedIn) return; 
      ...
      ..setNavigationDelegate(
        NavigationDelegate(
          ...
          onPageStarted: (String url) async {
            // 🔴 Çıkış URL'si Yakalama Kontrolü (Logout)
            final lowUrl = url.toLowerCase();
            if (lowUrl.contains('/logout') || lowUrl.contains('/cikis') || lowUrl.contains('/cikis-yap')) {
              final prefs = await SharedPreferences.getInstance();
              await prefs.remove(_kUserIdStorageKey);
              if (mounted) {
                setState(() {
                  _isLoggedIn = false; // Çıkış yapılınca reklam kilitlerini kapat
                });
              }
            }
  ...
  void _tryParseUserIdFromUrl(String url) async {
    try {
      ...
      // 🔴 Giriş başarılı: Reklam aktifleşti
      if (mounted) {
        setState(() {
          _isLoggedIn = true;
        });
        ...
        child: Scaffold(
          ...
          body: Stack(
            children: [
              ...
              // 🔴 Sadece bannerAdsEnabled VE isLoggedIn true ise banner göster
              if (_bannerAdsEnabled && _isLoggedIn)
